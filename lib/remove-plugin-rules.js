 ...
 * @returns {!object} A copy of {@link rules} with rules for
 * {@link pluginName} removed.
 */
export function rulesRemovePlugin(rules, pluginName) {
 ...
 * @param {string} pluginName Name of the plugin to remove.
 * @returns {!object} A copy of {@link configs} with rules for
 * {@link pluginName} removed from each configuration object.
 */
export function removePluginRules(configs, pluginName) {
  function removeRules(config) {
    ...
    return {
      ...config,
      "rules": rulesRemovePlugin(rules, pluginName)
    };
  }
